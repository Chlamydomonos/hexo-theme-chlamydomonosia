(function ()
{
    ...
    let archiveHeader = document.getElementsByClassName("archive-header")[0];

    function updatePosition()
    {
        if(cardsMargin.getBoundingClientRect().bottom - archiveHeader.getBoundingClientRect().height < cardsMargin.getBoundingClientRect().height / 2)
        {
            if(indexNav.style.position != "fixed")
            {
                let width = $("#profile-card").width();
                let top = archiveHeader.getBoundingClientRect().height + cardsMargin.getBoundingClientRect().height / 2;
